 ...
 */
 ...
 // generate p and q such that λ(n) = lcm(p - 1, q - 1) is coprime with e and |p-q| >= 2^(keysize - 100)

do {
	p = randomPrime(keysize / 2);
	q = randomPrime(keysize / 2);
	lambda = 
} while (bigInt.gcd(e, lambda).notEquals(1) || p.minus().abs().shiftRight(
